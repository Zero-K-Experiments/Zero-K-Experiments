-- Transifex helptext exporter
-- Has to be first to prevent autogenerated defs from interfering
-- Uncomment to use, seek the result in infolog
...
for id, ud in pairs(UnitDefs) do
	if first then
		...
		output = output .. ",\n" -- jason is retarded and won't accept {{},}
	end
	output = output .. "\t\"" .. (ud.unitname or "ERROR!") .. "\" : {\n"

	local name = string.gsub((ud.name or "ERROR!"), "\"", "\\\"")
	output = output .. "\t\t\"name\" : \"" .. name .. "\",\n"

	local description = string.gsub((ud.description or "ERROR!"), "\"", "\\\"")
	output = output .. "\t\t\"description\" : \"" .. description .. "\""

	if ud.customparams and ud.customparams.helptext then
		local helptext = string.gsub((ud.customparams.helptext or "ERROR!"), "\"", "\\\"")
		output = output .. ",\n\t\t\"helptext\" : \"" .. helptext .. "\"\n\t}"
	else
		output = output .. "\n\t}"
	end
...
output = output .. "\n}"
Spring.Echo("HELPTEXTS EXPORT DATA:\n" .. output)
]]
